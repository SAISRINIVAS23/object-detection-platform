...
import { FaUpload, FaDownload, FaRedo, FaExclamationTriangle, FaRobot, FaSlidersH } from "react-icons/fa";
...
function ImageDetection() {
    ...
    const handleUpload = async () => {
        ...
        try {
            const response = await api.post(`/detect/image?conf=${confidence}`, formData, {
                headers: {
            ...
            const event = new CustomEvent("new-notification", {
                detail: { text: `Image "${file.name}" processed: found ${response.data.detections?.length || 0} objects.` }
            });
            window.dispatchEvent(event);
        } catch (err) {
            console.error("Detection error:", err);
            setError(err.response?.data?.detail || "Object detection processing failed. Please try again.");
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleDownload = async () => {
        if (!result || !result.processed_image) return;
        try {
            const response = await fetch(result.processed_image);
            ...
            window.open(result.processed_image, "_blank");
        }
    };
    ...
    return (
        <div className="container-fluid py-4">
            <div className="mb-4">
                <h2 className="fw-bold text-dark m-0">Image Detection</h2>
                <p className="text-muted m-0">Upload an image to detect objects using YOLO11</p>
            </div>
            ...
            {!file ? (
                ...
                >
                    ...
                    <h5 className="fw-bold text-dark">Drag and drop your image here</h5>
                    <p className="text-muted mb-0">or click to browse from files</p>
                    <span className="text-muted mt-2" style={{ fontSize: "12px" }}>Supports JPG, PNG, WEBP</span>
                </div>
            ) : (
                /* Image selected / Results display */
                <div className="row g-4">
                    {/* Left/Middle Column: Images Display */}
                    <div className="col-lg-8">
                        <div className="card border-0 shadow-sm p-4 bg-white mb-4" style={{ borderRadius: "16px" }}>
                            <div className="d-flex justify-content-between align-items-center mb-3">
                                <h5 className="fw-bold text-dark m-0">
                                    {result ? "Annotated Result" : "Selected Preview"}
                                </h5>
                            ...
                            <div className="text-center bg-light rounded-3 p-2 border overflow-hidden position-relative d-flex align-items-center justify-content-center" style={{ minHeight: "400px" }}>
                                {loading && (
                                    <div className="position-absolute top-0 start-0 w-100 h-100 d-flex flex-column align-items-center justify-content-center bg-white bg-opacity-75 z-3">
                                        ...
                                        <h5 className="fw-bold text-primary">Running AI Detection...</h5>
                                        <p className="text-muted mb-0">Identifying objects with deep learning model</p>
                                    </div>
                                )}
                    ...
                    {/* Right Column: Detections Metadata & Sliders */}
                    <div className="col-lg-4">
                        <div className="card border-0 shadow-sm p-4 bg-white mb-4" style={{ borderRadius: "16px" }}>
                            ...
                            <div className="mb-2">
                                ...
                                />
                                <small className="text-muted">Higher values reduce false positives, lower values capture more items.</small>
                            </div>
                        </div>

                        <div className="card border-0 shadow-sm p-4 bg-white" style={{ borderRadius: "16px", minHeight: "250px" }}>
                            <h5 className="fw-bold text-dark mb-3">Detections Output</h5>
                                            ...
                                            {result.detections?.map((det, index) => (
                                                <div key={index} className="list-group-item d-flex justify-content-between align-items-center px-0 py-3 border-bottom">
                                                    <div>
                                                        ...
                                                        <div className="text-muted" style={{ fontSize: "12px" }}>Object #{index + 1}</div>
                                                    </div>
